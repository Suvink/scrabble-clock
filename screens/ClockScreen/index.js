...
const ClockScreen = ({ navigation }) => {
  ...
  const handlePlayPause = () => {
    isHapticsEnabled && Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
    if (!isGameStarted) {
    ...
    <SafeAreaView style={styles.safeAreaContainer}>
      <Layout style={parentLayoutStyles}>
        ...
        <View style={styles.settingsBar}>
          ...
          <Pressable style={styles.settingsButton} onPress={handlePlayPause}>
            {isGamePaused ? (
              <Ionicons name="play" size={50} color="white" style={{ padding: 0 }} />
            ) : (
        ...
        <Pressable
          style={bottomTimeEnded ? styles.clockActivePenalty : clockBottomRunning ? styles.clockActive : styles.clockView}
          onPress={handleBottomTap}
        >
          <View style={bottomTimeEnded ? styles.clockActivePenalty : clockBottomRunning ? styles.clockActive : styles.clockView}>
            <CountDown
              until={gameTime * 60}
      ...
      <Modal visible={resetModalVisible} backdropStyle={styles.backdrop}>
        <Card disabled={true} style={styles.modalCard}>
          <Text category="h6">Are you sure you want to reset the timer?</Text>
